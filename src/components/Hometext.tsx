const Hometext = () => {
  return (
    <div className="absolute top-64 w-full left-0">
      <div className="max-w-[1000px] md:pl-14 pl-9 m-auto justify-center items-center lg:items-start flex flex-col">
        <h3 className="max-md:text-[20px] font-marquis md:tracking-[1.5rem] tracking-[0.7rem] uppercase text-[40px] text-white">
          Edifico University
        </h3>
        <h4 className="font-name text-white md:text-[20px] md:pl-20 max-sm:pr-7 sm:pl-9 pt-6">
          Where learning meets excellence, empowering minds, achieving greatness
        </h4>
      </div>
    </div>
  );
};

export default Hometext;
